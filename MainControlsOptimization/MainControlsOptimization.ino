...
int toolDistance = 14000;
int toolDropDistance = 5000;
...
void tool1(void)
{
  ...
  double count = 0;
  for(int i=0; i<toolDropDistance; i++) //recalibrate it manually until we get a limit switch
  {
    digitalWrite(stepPin, HIGH);
    delayMicroseconds(stepDelay);
    digitalWrite(stepPin, LOW);
  ...
  stepper2.rotate(-toolDegrees);
  delay(3000);

  //insert actuator code here
  ...
  count = 0;
  digitalWrite(dirPin,HIGH);
  for(int i=0; i<toolDistance; i++) //recalibrate it manually until we get a limit switch
  {
    digitalWrite(stepPin, HIGH);
    delayMicroseconds(stepDelay);
    digitalWrite(stepPin, LOW);
